...
--
-- X is west-east, Z is north-south (-Z is north). Red spawns west (-X), Blue east (+X),
-- matching Forest and SnowFortress: MapService places each `Mirrored` piece first with the
-- prefix Red_ and no mirror, then Blue_ with mirror = "x", so authored coordinates ARE the
-- Red copy. The concept sketch labels the sides the other way round; only the labels moved.
--
