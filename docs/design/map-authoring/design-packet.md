...
| Everything is expressed relative to the source model's pivot. | Moving, turning or scaling the whole editable map is consistent by construction, and the playable map stays centred where MapService and terrain expect it. |
| Supported marker transforms: move, rotate an objective or spawn about vertical, scale a capture cylinder, move/scale the boundary, rotate the boundary by 90°, and move, turn or scale the whole map. **Refused with a message:** a tilted capture cylinder, a tilted spawn pad, or a boundary rotated by any angle other than a multiple of 90°. | Those can't become the axis-aligned data the runtime uses. |
...
| A07 | PASS (script) | Moving and turning the whole map leaves anchors unchanged. A widened boundary is regenerated, with a warning where ground stops short. A rotated boundary and a tilted cylinder are refused. |
